		...
		void __user *, arg)
{
#ifdef CONFIG_KSU_SUSFS
	/*
	 * susfs_stage6_trial88_raw_reboot_manager_bridge
	 * KSUN 3.3.0 ksud probes SUSFS with raw SYS_reboot.
	 * Trial88 services only Manager reporting commands here.
	 */
	if (magic1 == 0xDEADBEEF && magic2 == SUSFS_MAGIC &&
		...
		pr_info("Trial88 SUSFS RAW REBOOT: cmd=0x%x uid=%u\n",
			cmd, current_uid().val);

		switch (cmd) {
		case CMD_SUSFS_SHOW_ENABLED_FEATURES:
			pr_info("Trial88 SUSFS RAW REBOOT: SHOW_ENABLED_FEATURES\n");
			susfs_get_enabled_features(susfs_arg);
			return 0;
		case CMD_SUSFS_SHOW_VARIANT:
			pr_info("Trial88 SUSFS RAW REBOOT: SHOW_VARIANT\n");
			susfs_show_variant(susfs_arg);
			return 0;
		case CMD_SUSFS_SHOW_VERSION:
			pr_info("Trial88 SUSFS RAW REBOOT: SHOW_VERSION\n");
