...
{
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	// We should first check the 'SUSFS_MNT_ID_BACKUP(mnt)', see if it is DEFAULT_SUS_MNT_ID_FOR_KSU_PROC_UNSHARE
	// if so, these mnt_id were not assigned by mnt_alloc_id() so we don't need to free it.
	if (unlikely(SUSFS_MNT_ID_BACKUP(mnt) == DEFAULT_SUS_MNT_ID_FOR_KSU_PROC_UNSHARE)) {
		return;
	}
