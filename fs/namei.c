...
static inline int may_follow_link(struct nameidata *nd)
{
	...
	if (nd->inode && unlikely(nd->inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &nd->inode->i_mapping->flags)) && likely(susfs_is_current_proc_umounted_app())) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (link->dentry->d_inode && unlikely(link->dentry->d_inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &link->dentry->d_inode->i_mapping->flags)) && likely(susfs_is_current_proc_umounted_app())) {
		return -ENOENT;
	}
#endif
...
{
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &inode->i_mapping->flags)) && likely(susfs_is_current_proc_umounted_app())) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && dentry->d_inode && unlikely(dentry->d_inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &dentry->d_inode->i_mapping->flags)) && likely(susfs_is_current_proc_umounted_app())) {
		dput(dentry);
		return ERR_PTR(-ENOENT);
	}
#endif
			   ...
			   struct path *path, struct path *root)
{
	...
	restore_nameidata();
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && path->dentry->d_inode && unlikely(path->dentry->d_inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &path->dentry->d_inode->i_mapping->flags)) && likely(susfs_is_current_proc_umounted_app())) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode &&
	    unlikely(child->d_inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &child->d_inode->i_mapping->flags)) &&
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode && unlikely(dentry->d_inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &dentry->d_inode->i_mapping->flags)) && likely(susfs_is_current_proc_umounted_app())) {
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (unlikely(dentry->d_inode->i_mapping && test_bit(AS_FLAGS_SUS_PATH, &dentry->d_inode->i_mapping->flags)) && likely(susfs_is_current_proc_umounted_app())) {
			dput(dentry);
			return -ENOENT;
		}
#endif
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	if (!IS_ERR(filp) && unlikely(filp->f_inode->i_mapping && test_bit(AS_FLAGS_OPEN_REDIRECT, &filp->f_inode->i_mapping->flags)) && current_uid().val < 2000) {
